...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _handleSend(String text) async {
    ...
    try {
      // Send everything before this new message as memory context. The
      // service internally trims this to the last N turns (config.dart)
      // so cost doesn't grow unbounded as the chat gets longer.
      final priorHistory = _messages.sublist(0, _messages.length - 1);
      final reply = await _gemini.sendMessage(text, history: priorHistory);
      setState(() {
